...
def get_next_saturday_str():
    ...
    if days_ahead == 0: days_ahead = 7 # Se hoje for sábado, pega o próximo
    next_sat = now + timedelta(days=days_ahead)
    return next_sat.strftime("%d/%m")

def get_next_saturday_iso():
    now = datetime.now()
    days_ahead = (5 - now.weekday() + 7) % 7
    if days_ahead == 0: days_ahead = 7
    next_sat = (now + timedelta(days=days_ahead)).date()
...
def tuesday_repertoire(event: scheduler_fn.ScheduledEvent) -> None:
    ...
    if state_doc.exists and state_doc.to_dict().get("cancelled_date") == sat_iso:
...
# 7. AGENDAMENTO: MONITOR DE ENQUETES (CADA 1 HORA)
